...
public class ReporteCategoriaControlador extends GenericForwardComposer<Component>
{
	...
	public void onClick$buttonAceptarA(){
		...
		ArrayList<ReportesCategoriaentidad> lista = dbrp.ReportePorAño(cmb_anio.getText());
	...
	public void onClick$buttonAceptarP(){
		...
		ArrayList<ReportesCategoriaentidad> lista = dbrp.ReportePorMes((cmb_mes.getSelectedIndex()+1),cmb_anio2.getText());
		ListModelList<ReportesCategoriaentidad> modeloDeDatos= new ListModelList<ReportesCategoriaentidad>(lista);
		listademanda.setModel(modeloDeDatos);
		gpb_lista.setVisible(true);
		...
		else
		{
			...
			if (txtFechaLlegada.getValue().after(txtFechaSalida.getValue())){
				...
				if(txtFechaLlegada.getValue().after(c1.getTime()) || txtFechaSalida.getValue().after(c1.getTime())){
					...
					ArrayList<ReportesCategoriaentidad> lista = dbrp.ReportePorFecha(fechai,fechau);
					ListModelList<ReportesCategoriaentidad> modeloDeDatos= new ListModelList<ReportesCategoriaentidad>(lista);
					listademanda.setModel(modeloDeDatos);
					gpb_lista.setVisible(true);
